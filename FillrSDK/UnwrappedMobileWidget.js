...
var Affiliate, Fields, Mappings, Pop, Preferences, PublisherApi, Values, perfWrap;

Mappings = require('widget/pop/mappings');

Fields = require('widget/fields');

Values = require('widget/values');
...
module.exports = {
  getFields: function() {
    ...
    if (typeof Event === "function" && typeof document.dispatchEvent === "function") {
      ev = new Event("Fillr:getFields:done");
      document.dispatchEvent(ev);
    }
    return p;
  },
  widgetFields: function() {
    return Fields.fields;
  },
  getValues: function() {
    var p;
    p = perfWrap('getValues', function() {
      return Values.getValues();
    });
    if (Preferences.debug) {
      console.log("getValues", p);
    }
    return p;
  },
  ...
  populateWithMappings: function(mappedFields, popData, previewFill) {
    var dataKeys, e, ev, key, res, value;
    if (previewFill == null) {
      previewFill = false;
    }
    if (Preferences.debug) {
      console.log("fill", mappedFields, popData, previewFill);
    }
    res = null;
    try {
      ...
      if (typeof Event === 'function' && typeof document.dispatchEvent === 'function') {
        ev = new Event('Fillr:fill:done');
        ev.mappings = mappedFields;
        dataKeys = [];
        for (key in popData) {
          value = popData[key];
          if ((value != null) && value !== '') {
...
require.register("widget/events", function(exports, require, module) {
var AndroidDOMEventInterface, Controller, FillrDOMEvents, GeckoDOMEventInterface, Json, Preferences, fillr_listeners, iOSDOMEventInterface, removePreWidgetEvents,
...
removePreWidgetEvents = function(win) {
  if (win.FillrFieldFocusEvent != null) {
    console.log("FillrSDK: Removing temp events");
    win.document.removeEventListener('click', win.FillrClickedEvent, true);
    win.document.removeEventListener('focus', win.FillrFieldFocusEvent, true);
    win.document.removeEventListener('focusout', win.FillrFieldFocusOutEvent, true);
    win.removeEventListener('orientationchange', win.FillrOrientationChangeEvent, true);
    return delete win.FillrFieldFocusEvent;
  }
};
...
module.exports = {
  setup: function(win1) {
    this.win = win1;
    removePreWidgetEvents(this.win);
    return this._attach(this.win);
  },
  _checkTarget: function(e) {
    if (typeof e === 'undefined' || e === null || typeof e.nodeName !== 'string') {
      return false;
    }
    return true;
  },
  _isInput: function(e) {
    var el;
    el = e.nodeName.toLowerCase();
    if (el === 'input' || el === 'select' || el === 'password' || el === 'textarea') {
      return true;
    }
    return false;
  },
  _attach: function(win) {
    var ref, ref1;
    console.log("FillrSDK: Attaching DOM events..." + (win != null ? (ref = win.document) != null ? (ref1 = ref.location) != null ? ref1.href : void 0 : void 0 : void 0));
    win.document.removeEventListener('click', fillr_listeners.clicked, true);
    win.document.removeEventListener('focus', fillr_listeners.focus, true);
    win.document.removeEventListener('blur', fillr_listeners.focus_out, true);
    win.removeEventListener('orientationchange', fillr_listeners.orientation_change, true);
    win.removeEventListener('message', fillr_listeners.message_received, true);
    fillr_listeners.clicked = this._handlers.FillrClickedEvent.bind(this);
    fillr_listeners.focus = this._handlers.FillrFieldFocusEvent.bind(this);
    fillr_listeners.focus_out = this._handlers.FillrFieldFocusOutEvent.bind(this);
    fillr_listeners.orientation_change = this._handlers.FillrOrientationChangeEvent.bind(this);
    fillr_listeners.message_received = this._handlers.ReceiveMessage.bind(this);
    win.document.addEventListener('click', fillr_listeners.clicked, true);
    win.document.addEventListener('focus', fillr_listeners.focus, true);
    win.document.addEventListener('blur', fillr_listeners.focus_out, true);
    win.addEventListener('orientationchange', fillr_listeners.orientation_change, true);
    return win.addEventListener('message', fillr_listeners.message_received, false);
  },
  _handlers: {
    ReceiveMessage: function(event) {
      var removeChip;
      removeChip = (function(_this) {
        return function() {
          var fillrChip;
          fillrChip = _this.win.document.getElementById('fillr-chip');
          return fillrChip.parentNode.removeChild(_this.win.document.getElementById('fillr-chip'));
        };
      })(this);
      if (event.data === 'fillr_chip_clicked') {
        removeChip();
        this.win.PopWidgetInterface.getFields();
      }
      if (event.data === 'fillr_chip_closed') {
        return removeChip();
      }
    },
    FillrClickedEvent: function(e) {
      if ((e != null ? e.isFillrWidgetEvent : void 0) === true) {
        return;
      }
      if (this._isInput(this.win.document.activeElement)) {
        console.log('FillrClicked: Field focussed');
        this._interface().click();
      }
    },
    FillrFieldFocusEvent: function(e) {
      if ((e != null ? e.isFillrWidgetEvent : void 0) === true) {
        return;
      }
      ...
      if (this._isInput(e.target)) {
        console.log('FillrFocus: Field focussed');
        this._interface().focus();
      }
    },
    FillrFieldFocusOutEvent: function(e) {
      if ((e != null ? e.isFillrWidgetEvent : void 0) === true) {
        return;
      }
      if (!this._checkTarget(e.target)) {
      ...
      if (this._isInput(e.target)) {
        console.log('FillrFocusOut: Field focussed out');
        this._interface().focusout();
      }
    },
    FillrOrientationChangeEvent: function(e) {
      if (this._isInput(this.win.document.activeElement)) {
        console.log('FillrOrientation: Field focussed');
        this._interface().orientationchange();
      }
    }
  },
  _interface: function() {
    ...
    if (typeof androidInterface !== 'undefined' && typeof androidInterface.fieldFocussed === 'function') {
      this["interface"] = new AndroidDOMEventInterface();
    } else if (Preferences.browserType === 'Fennec') {
      this["interface"] = new GeckoDOMEventInterface();
    } else {
      this["interface"] = new iOSDOMEventInterface();
    }
    return this["interface"];
  }
};

FillrDOMEvents = (function() {
  function FillrDOMEvents() {}

  FillrDOMEvents.prototype.focus = function() {
    return void 0;
  };

  FillrDOMEvents.prototype.click = function() {
    return void 0;
  };

  FillrDOMEvents.prototype.focusout = function() {
    return void 0;
  };

  FillrDOMEvents.prototype.orientationchange = function() {
    return void 0;
  };

  return FillrDOMEvents;

})();

GeckoDOMEventInterface = (function(superClass) {
  extend(GeckoDOMEventInterface, superClass);

  function GeckoDOMEventInterface() {
    return GeckoDOMEventInterface.__super__.constructor.apply(this, arguments);
  }

  GeckoDOMEventInterface.prototype.focus = function() {
    console.log("FillrSDK:Widget. Focus");
    return typeof Messaging !== "undefined" && Messaging !== null ? typeof Messaging.sendRequest === "function" ? Messaging.sendRequest({
      type: "Fillr:FieldFocussed"
    }) : void 0 : void 0;
  };

  GeckoDOMEventInterface.prototype.click = function() {
    console.log("FillrSDK:Widget. Click");
    return typeof Messaging !== "undefined" && Messaging !== null ? typeof Messaging.sendRequest === "function" ? Messaging.sendRequest({
      type: "Fillr:FieldFocussed"
    }) : void 0 : void 0;
  };

  GeckoDOMEventInterface.prototype.focusout = function() {
    console.log("FillrSDK:Widget. Blur");
    if (typeof Messaging !== "undefined" && Messaging !== null) {
      if (typeof Messaging.sendRequest === "function") {
        Messaging.sendRequest({
          type: "Fillr:FieldBlurred"
        });
      }
    }
    return typeof Messaging !== "undefined" && Messaging !== null ? typeof Messaging.sendRequest === "function" ? Messaging.sendRequest({
      type: "Fillr:SetValues",
      fields: Controller.getFields(),
      values: Controller.getValues()
    }) : void 0 : void 0;
  };

  GeckoDOMEventInterface.prototype.orientationchange = function() {
    console.log("FillrSDK:Widget. Orientation Change");
    return typeof Messaging !== "undefined" && Messaging !== null ? typeof Messaging.sendRequest === "function" ? Messaging.sendRequest({
      type: "Fillr:FieldFocussed"
    }) : void 0 : void 0;
  };

  return GeckoDOMEventInterface;

})(FillrDOMEvents);

AndroidDOMEventInterface = (function(superClass) {
  extend(AndroidDOMEventInterface, superClass);

  function AndroidDOMEventInterface() {
    return AndroidDOMEventInterface.__super__.constructor.apply(this, arguments);
  }

  AndroidDOMEventInterface.prototype.focus = function() {
    return typeof androidInterface.fieldFocussed === "function" ? androidInterface.fieldFocussed(Json.stringify({})) : void 0;
  };

  AndroidDOMEventInterface.prototype.click = function() {
    return typeof androidInterface.fieldFocussed === "function" ? androidInterface.fieldFocussed(Json.stringify({})) : void 0;
  };

  AndroidDOMEventInterface.prototype.focusout = function() {
    if (typeof androidInterface.fieldBlurred === "function") {
      androidInterface.fieldBlurred('{}');
    }
    return typeof androidInterface.setValues === "function" ? androidInterface.setValues(Json.stringify(Controller.getFields()), Json.stringify(Controller.getValues())) : void 0;
  };

  AndroidDOMEventInterface.prototype.orientationchange = function() {
    return typeof androidInterface.fieldFocussed === "function" ? androidInterface.fieldFocussed(Json.stringify({})) : void 0;
  };

  return AndroidDOMEventInterface;

})(FillrDOMEvents);

iOSDOMEventInterface = (function(superClass) {
  extend(iOSDOMEventInterface, superClass);

  function iOSDOMEventInterface() {
    return iOSDOMEventInterface.__super__.constructor.apply(this, arguments);
  }

  iOSDOMEventInterface.prototype.focus = function() {
    return window.location.href = 'fillr:focus';
  };

  iOSDOMEventInterface.prototype.click = function() {
    return window.location.href = 'fillr:focus';
  };

  iOSDOMEventInterface.prototype.focusout = function() {
    return window.location.href = 'fillr:blur';
  };

  iOSDOMEventInterface.prototype.orientationchange = function() {
    return window.location.href = 'fillr:focus';
  };

  return iOSDOMEventInterface;
...
var ErrorCodes, FillrId, Fom, FormInput, IsVisible, SectionHint, jQuery;
...
FillrId = require('widget/fields/fillr_id');
...
module.exports = {
  ...
  _detect: function(searchRoot) {
    ...
    this._assignFillrIds(allFields);
    this.fom = Fom.create();
    fields = (function() {
      var j, len, results;
      results = [];
  ...
  _assignFillrIds: function(fields) {
    var field, j, len, results;
    results = [];
    for (j = 0, len = fields.length; j < len; j++) {
      field = fields[j];
      results.push(FillrId.get(field));
    }
    return results;
  },
  _allFields: function() {
    ...
    err = 0x0;
    selectors = ['input:not([type=button]):not([type=submit]):not([type=reset]):not([type=checkbox]):not([type=image]):not([readonly=readonly]):not([role=button]):not([name=q]):not([type=search])', 'select', 'textarea'].join(', ');
    things = [];
...
require.register("widget/fields/fillr_id", function(exports, require, module) {
module.exports = {
  attribute: 'data-fillr-id',
  get: function(el) {
    var ref;
    if ((el != null ? (ref = el.attributes[this.attribute]) != null ? ref.value : void 0 : void 0) == null) {
      if (el != null) {
        el.setAttribute(this.attribute, this._popID());
      }
    }
    return parseInt(el.attributes[this.attribute].value);
  },
  _popID: function() {
    return Math.floor((1 + Math.random()) * 0x10000);
  },
  regex: /\s?data-fillr-id=\"[0-9]*\"\s?/gi
};

});

require.register("widget/fields/fom", function(exports, require, module) {
var FillrId, Preferences, _cloneNode, _elementNode, _nodeFilter,
...
module.exports = {
  ...
  create: function(force) {
    ...
    Array.prototype.slice.call(this.dom.querySelectorAll('p,div,span,small')).map((function(_this) {
      return function(aNode) {
        ...
        if (_this.errorRgx.test(aNode.getAttribute('class')) && aNode.querySelectorAll('input,select').length === 0) {
          aNode.parentElement.removeChild(aNode);
        }
      };
    })(this));
  ...
  indexOf: function(el) {
    ...
    index = this.fom.indexOf(outerHtml);
    if (index > -1) {
      return index;
    }
    outerHtml = outerHtml.replace(FillrId.regex, '');
    index = this.fom.indexOf(outerHtml);
    return index;
  },
  processHeadings: function() {
    var avg, exclude, n, px, ref, s, sum, t, walk;
    ...
    avg = sum / px.length;
    if (Preferences.debug) {
      console.log('Heading average threshold:', avg);
    }
    ...
    if (Preferences.debug) {
      return console.log("Headings: ", this.headings);
    }
  }
};
...
module.exports = LabelHelper = (function() {
  ...
  LabelHelper.prototype.process = function() {
    var detectedLabel, e, i, j, label, labels, len, len1, ref, ref1, strategy;
    ...
    if (labels.length > 0) {
      ...
      for (j = 0, len1 = ref1.length; j < len1; j++) {
        strategy = ref1[j];
        detectedLabel = strategy.call(this);
        label = this.trim((detectedLabel != null ? detectedLabel.label : void 0) || detectedLabel);
        if (label) {
          if (this._valid(label)) {
            this.confidence = 0.1;
            this.label = label;
            this.before_text = detectedLabel != null ? detectedLabel.above : void 0;
            this.after_text = detectedLabel != null ? detectedLabel.below : void 0;
            return;
          }
        }
      }
...
var FillrId, Label, Legend, MetaData, jQuery;
...
module.exports = MetaData = (function() {
  function MetaData(el) {
    ...
    this.pop_id = FillrId.get(el);
    lh = Label.detect(el);
    this.label = lh.label;
    this.label_confidence = lh.confidence;
    this.before_text = lh != null ? (ref = lh.before_text) != null ? ref.substring(0, 50) : void 0 : void 0;
    this.after_text = lh != null ? (ref1 = lh.after_text) != null ? ref1.substring(0, 50) : void 0 : void 0;
    ...
    this.classes = this._value(el, 'class');
  }
...
module.exports = {
  hints: ['bill', 'ship', 'postal', 'delivery', 'residential', 'payment', 'birth', /[^a-zA-Z0-9]*age[^a-zA-Z0-9]*/, '收货', '发货', '联系信息', '联系方式', '公司信息', '企业信息', '账单', '工作信息'],
  ...
  _checkHint: function(line) {
    var h, hint, i, j, k, l, len, len1, matched, ref, x;
    l = line.toLowerCase();
    ref = this.hints;
    for (i = j = 0, len = ref.length; j < len; i = ++j) {
      hint = ref[i];
      matched = this._matchHint(l, hint);
      if (matched) {
        x = this.hints.slice();
        x.splice(i, 1);
        for (k = 0, len1 = x.length; k < len1; k++) {
          h = x[k];
          if (this._matchHint(l, h)) {
            return null;
          }
        }
...
require.register("widget/fill_prompt", function(exports, require, module) {
module.exports = {
  html: "<html>\n<head>\n  <style>\n    body {\n      margin: 0;\n      padding: 0 7px 7px 0;\n    }\n    #container {\n      background-color: #F0F0F0;\n      display: flex;\n      opacity: 0;\n      width: 276px;\n      height: 48px;\n    }\n    #autofill {\n      flex: 2;\n      cursor: pointer;\n      border: none;\n    }\n    #logo {\n      width: 21px;\n      height: 23px;\n      margin: 5px 0 6px 12px;\n      vertical-align: middle;\n    }\n    #text {\n      line-height: 48px;\n      padding-left: 20px;\n      font-family: Roboto;\n      font-size: 14px;\n      color: rgba(0,0,0,0.87);\n    }\n    #cancel {\n\n    }\n    #cancel-image {\n      margin: 11px 12px 14px 0;\n      width: 24px;\n      height: 24px;\n      cursor: pointer;\n      border: none;\n      vertical-align: middle;\n    }\n  </style>\n</head>\n<body>\n  <div id='container'>\n    <a id='autofill'>\n      <img id='logo' src='https://fillr.s3.amazonaws.com/desktop-extension/assets/fillr-icon.png' />\n      <span id='text'>\n        Autofill this form\n      </span>\n    </a>\n    <a id='cancel'>\n      <img id='cancel-image' src='https://fillr.s3.amazonaws.com/desktop-extension/assets/cancel.png' />\n    </a>\n  </div>\n  <script type='text/javascript'>\n    var container = document.getElementById('container');\n    setTimeout(function(){\n      container.setAttribute('style', 'opacity: 1; transition: opacity 0.3s linear;');\n    }, 50);\n\n    var autofill = document.getElementById('autofill');\n    autofill.onclick = function() {\n      window.parent.postMessage('fillr_chip_clicked', '*');\n    }\n\n    var cancel = document.getElementById('cancel');\n    cancel.onclick = function() {\n      window.parent.postMessage('fillr_chip_closed', '*');\n    }\n  </script>\n</body>\n</html>",
  closeIframeTimeoutID: null,
  suppressInFieldFill: false,
  chipField: null,
  maxZIndex: 2147483647,
  padding: 4,
  scrollBarSize: 15,
  frameWidth: 274,
  iframeShadow: "0 0 8px 0 rgba(0,0,0,0.12), 0 8px 8px 0 rgba(0,0,0,0.24)",
  wrapEvent: function(htmlElement, eventName, fn) {
    var originalHandler;
    originalHandler = htmlElement[eventName];
    return htmlElement[eventName] = (function(_this) {
      return function(event) {
        fn();
        if (originalHandler) {
          return originalHandler.call(htmlElement, event);
        }
      };
    })(this);
  },
  addInFieldFocus: function(htmlElement) {
    this.wrapEvent(htmlElement, 'oninput', (function(_this) {
      return function() {
        return _this.showInPageFillChip(htmlElement);
      };
    })(this));
    this.wrapEvent(htmlElement, 'onfocus', (function(_this) {
      return function() {
        return _this.showInPageFillChip(htmlElement);
      };
    })(this));
    return this.wrapEvent(htmlElement, 'onblur', (function(_this) {
      return function() {
        _this.iframe.setAttribute("style", _this.iframe.getAttribute("style") + "; opacity: 0;  transition: opacity 400ms linear;");
        return _this.closeIframeTimeoutID = window.setTimeout(function() {
          if (_this.chipField === htmlElement) {
            return _this.closeIframe();
          }
        }, 1000);
      };
    })(this));
  },
  inFieldPromptEnabled: function() {
    return true;
  },
  show: function(field) {
    var autofill, body, cancel, chipHeight, chipWidth, container, fieldRect, scrollLeft, scrollTop, x, y;
    if ((this.chipField === field) || this.suppressInFieldFill) {
      return;
    }
    ...
    this.closeIframe();
    this.chipField = field;
    if (this.inFieldPromptEnabled()) {
      body = document.getElementsByTagName("body")[0];
      scrollTop = document.body.scrollTop;
      scrollLeft = document.body.scrollLeft;
      this.iframe = document.createElement("iframe");
      this.iframe.src = "data:text/html;charset=utf-8," + escape(this.html);
      this.iframe.setAttribute("scrolling", "no");
      this.iframe.setAttribute("id", "fillr-chip");
      fieldRect = field.getBoundingClientRect();
      scrollTop = document.body.scrollTop;
      scrollLeft = document.body.scrollLeft;
      chipWidth = this.frameWidth;
      chipHeight = 48;
      x = fieldRect.left + scrollLeft;
      y = fieldRect.bottom + scrollTop + this.padding;
      if ((x + chipWidth) > (scrollLeft + (window.innerWidth - this.scrollBarSize))) {
        x = (scrollLeft + (window.innerWidth - this.scrollBarSize)) - chipWidth - this.padding;
      }
      if ((y + chipHeight) > (scrollTop + (window.innerHeight - this.scrollBarSize))) {
        y = fieldRect.top + scrollTop - chipHeight - this.padding;
      }
      this.iframe.setAttribute("style", "position: absolute; background: transparent; left: " + x + "px; top: " + y + "px; width: " + chipWidth + "px; height: " + chipHeight + "px; border: none; z-index: " + this.maxZIndex + "; box-shadow: " + this.iframeShadow + ";");
      this.iframe.onblur = this.closeIframe;
      body.appendChild(this.iframe);
      container = document.getElementById('container');
      setTimeout((function(_this) {
        return function() {
          return container.setAttribute('style', 'opacity: 1; transition: opacity 0.3s linear;');
        };
      })(this), 50);
      autofill = document.getElementById('autofill');
      autofill.onclick = (function(_this) {
        return function() {};
      })(this);
      cancel = document.getElementById('cancel');
      return cancel.onclick = (function(_this) {
        return function() {
          return _this.closeIframe();
        };
      })(this);
...
require.register("widget/filter_values", function(exports, require, module) {
var _isPlaceholder, indicators;

indicators = [/day/i, /month/i, /year/i, /select/i, /please pick one/i, /title/i, /country/i, /state/i, /honorific/i];

module.exports = {
  filterValues: function(fields, values) {
    var field, i, len, ref, ref1;
    for (i = 0, len = fields.length; i < len; i++) {
      field = fields[i];
      if (((ref = field.metadata) != null ? ref.tag_name : void 0) === 'select') {
        if (((ref1 = field.el[0]) != null ? ref1.value : void 0) === values[field.metadata.pop_id]) {
          if (_isPlaceholder(values[field.metadata.pop_id])) {
            delete values[field.metadata.pop_id];
...
var Controller, DomEvents, Json, Prompt;

Json = require('widget/lib/json');

Controller = require('widget/controller');

Prompt = require('widget/fill_prompt');

DomEvents = require('widget/events');
...
module.exports = {
  ...
  checkFields: function() {
    return androidInterface.checkFields(Json.stringify(Controller.getFields()));
  },
  populateWithMappings: function(mappedFields, popData) {
    return Controller.populateWithMappings(mappedFields, popData);
  },
  showPrompt: function() {
    return Prompt.show(document.activeElement);
  },
  setupEvents: function() {
    return DomEvents.setup(window);
  },
  require: function(args) {
    return require(args);
  }
...
module.exports = {
  ...
  getValues: function() {
    return Json.stringify(Controller.getValues());
  },
  populateWithMappings: function(mappedFields, popData) {
    return Controller.populateWithMappings(mappedFields, popData);
  },
...
;require.register("widget/lib/fingerprint", function(exports, require, module) {
...
module.exports = {
  ...
  create: function(args) {
    var c, custom, e, field, fields, i, j, k, len, len1, len2, mappedField, newValue, o, post, pre, ref, ref1, sortedMappedFields, unfilled_fields;
    this.args = args;
    fields = [];
    unfilled_fields = [];
    ...
    if (((ref = this.args) != null ? ref.previewFill : void 0) === true) {
      this.previewFill = true;
    }
    ...
    for (i = 0, len = sortedMappedFields.length; i < len; i++) {
      ...
      if (field = this._findField(mappedField.pop_id)) {
        ...
        newValue = this._newValue(mappedField);
        if (field.param !== 'ignore' && newValue && newValue.trim() !== "") {
    ...
    for (k = 0, len2 = fields.length; k < len2; k++) {
      ...
      try {
        ...
        e = error;
        if (Preferences.debug) {
          console.log("Could not fill", field.metadata.name || field.metadata.id);
        }
      }
    }
    if (post) {
  ...
  _popField: function(field, reload) {
    ...
    helper = Helper.run(field, reload, this.previewFill);
    if (helper.filled) {
      if (Preferences.debug) {
...
module.exports = FieldPopHelper = (function() {
  FieldPopHelper.prototype.previewFill = false;

  function FieldPopHelper(field1, reload1, previewFill1) {
    this.field = field1;
    this.reload = reload1 != null ? reload1 : false;
    this.previewFill = previewFill1 != null ? previewFill1 : false;
  }

  FieldPopHelper.factory = function(field, reload, previewFill) {
    var helper, obj;
    if (reload == null) {
      reload = false;
    }
    ...
    obj = new FieldPopHelper(field, reload, previewFill);
  ...
  FieldPopHelper.run = function(field, reload, previewFill) {
    var helper, obj;
    if (reload == null) {
      reload = false;
    }
    ...
    obj = new FieldPopHelper(field, reload, previewFill);
    if (helper = obj.createHelper()) {
      helper.previewFill = previewFill;
      if (reload) {
        helper.reload();
      }
...
module.exports = BaseField = (function() {
  ...
  BaseField.prototype._browserChangeEvents = function() {
    if (Preferences.browserType === 'Fennec') {
      return ['keyup', 'input', 'change', 'blur'];
    } else if (Preferences.browserType === 'Desktop' && this.previewFill === true) {
      return ['keypress', 'keydown', 'keyup', 'input', 'change'];
    } else {
      return ['keypress', 'keydown', 'keyup', 'input', 'change', 'blur'];
    }
  };

  BaseField.prototype.doChange = function(el) {
    var evName, evtA, evtB, k, len, ref, ref1, ref2;
    ref = this._browserChangeEvents();
    for (k = 0, len = ref.length; k < len; k++) {
      evName = ref[k];
      evtA = document.createEvent('HTMLEvents');
      evtA.popGenerated = true;
      evtA.initEvent(evName, true, true);
      evtA.isFillrWidgetEvent = true;
...
module.exports = RadioField = (function(superClass) {
  ...
  function RadioField(field) {
    var item, ref1, ref2, ref3, ref4;
    RadioField.__super__.constructor.call(this, field);
    this.initialValue = (ref1 = this.field) != null ? (ref2 = ref1.el) != null ? ref2.value : void 0 : void 0;
    this.group = document.querySelectorAll("input[name='" + ((ref3 = this.field) != null ? (ref4 = ref3.el) != null ? ref4.name : void 0 : void 0) + "'][type=radio]");
  ...
  RadioField.detect = function(field) {
    var ref1, ref2;
    return (field != null ? (ref1 = field.el) != null ? ref1.tagName : void 0 : void 0) === 'INPUT' && (field != null ? (ref2 = field.el) != null ? ref2.type : void 0 : void 0) === 'radio';
  };
  ...
  RadioField.prototype._assignAndCheck = function(val) {
    ...
    if ((this.group != null) && this.group.length > 0) {
      ...
      for (i = 0, len = ref1.length; i < len; i++) {
        el = ref1[i];
        if ((el != null ? el.value : void 0) === val) {
          if (el != null) {
            el.checked = true;
          }
          return true;
...
module.exports = TwoDigitsField = (function(superClass) {
  ...
  TwoDigitsField.prototype._pad = function(n, width, z) {
    z = z || '0';
    n = n + '';
    if (n.length >= width) {
      return n;
...
require.register("widget/values", function(exports, require, module) {
var FillrId, FilterValues, _detectValues, _isInitialPageGetValues, cached_values, is_initial_page_getvalues;

FillrId = require('widget/fields/fillr_id');

FilterValues = require('widget/filter_values');

cached_values = null;

is_initial_page_getvalues = true;

module.exports = {
  setFields: function(fields) {
    var filtered_values, new_values;
    new_values = _detectValues(fields);
    is_initial_page_getvalues = _isInitialPageGetValues(new_values);
    filtered_values = FilterValues.filterValues(fields, new_values);
    return cached_values = filtered_values;
  },
  getValues: function() {
    var wrapped_values;
    wrapped_values = {};
    wrapped_values["is_initial_page_getvalues"] = is_initial_page_getvalues;
    is_initial_page_getvalues = false;
    wrapped_values["values"] = cached_values;
    return wrapped_values;
  }
};

_detectValues = function(fields) {
  var field, i, key, len, values;
  values = {};
  for (i = 0, len = fields.length; i < len; i++) {
    field = fields[i];
    key = field.el.attributes[FillrId.attribute].value;
    values[key] = field.el.value;
  }
  return values;
};

_isInitialPageGetValues = function(new_values) {
  var i, key, len, ref;
  if (cached_values === null) {
    return true;
  }
  if (Object.keys(new_values).length !== Object.keys(cached_values).length) {
    return true;
  }
  ref = Object.keys(new_values);
  for (i = 0, len = ref.length; i < len; i++) {
    key = ref[i];
    if (!cached_values.hasOwnProperty(key)) {
      return true;
    }
  }
  return false;
...
module.exports = { "version" : "1.15.27" };
});


window.PopWidgetInterface = require('widget/interfaces/ios_sdk'); window.PopWidgetInterface.require('widget/events').setup(window); }); if(typeof window.PopWidgetInterface == 'undefined') { FillrWidget.call({}); var style = document.createElement("style"); style.setAttribute("rel", "stylesheet"); style.setAttribute("type", "text/css"); style.appendChild(document.createTextNode(".pop-field { transition: background-color 3s; background-color: default; } .pop-highlight { transition: none !important; background-color: rgba(0, 164, 184, 0.5); }")); document.head.appendChild(style); }