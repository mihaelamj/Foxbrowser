...
- (void)_addStringURLBookmark:(NSString *)stringURL title:(NSString *)title {
  NSURL *url = [NSURL URLWithString:stringURL];
  //  FXSyncItem *item = [[FXSyncStock sharedInstance] bookmarkForUrl:url];
  [[FXSyncStock sharedInstance] newBookmarkWithTitle:title url:url];
}

- (void)makeFillrBookmarks {
  
  if (_fillrItemsCreated) {
    return;
  }
  
  NSArray *items = @[@"https://psr.slv.vic.gov.au",
                     @"http://zale.sheldoncollege.com//index.php?sid=16591",// App tansport security
  ...
  for (NSString *item in items) {
    ...
    NSString *title = [NSString stringWithFormat:@"Fillr item: %i",i];
    [self _addStringURLBookmark:item title:title];
    
  }
  
  _fillrItemsCreated = true;
